  ...
  return (
    <Autocomplete
      ...
      PaperComponent={({ children }) => (
        <Paper style={{ width: 400 }}>{children}</Paper>
      )}
      ...
      renderInput={(params) => {
        let newValue = params.inputProps.value;
        const exists = countries.find((c) => {
          return c.label === newValue;
        });
        if (exists) {
          setSelectedCountryCode(exists.phone);
          newValue = "+" + exists.phone;
        }
        return (
          <TextField
            {...params}
            ...
            style={{ width: 125 }} // Ancho del select
          />
        );
      }}
    />
